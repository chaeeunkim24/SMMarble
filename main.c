...
void generatePlayers(int n, int initEnergy)
{
     ...
     for (i=0;i<player_nr;i++)  // n is the number of players
     {
     player_pos[i] = 0;
     player_credit[i] = 0;
     
     printf("input %i-th player name: ");  //can use do-while
     scanf("%s", &player_name[i][0]);
     
     player_energy[i] = initEnergy;  //called by out of this func
...
int main(int argc, const char * argv[]) {
    ...
    do
    {
        ...
        scanf("%i", &player_nr);  // input the number of the player
        fflush(stdin);  // remove the buffer => if not remove the buffer, it could bring out the error
        
        if (player_nr <= 0 || player_nr > MAX_PLAYER)
           printf("Invalid player number!\n");
    }
    while (player_nr <= 0 || player_nr > MAX_PLAYER);  // in this case, can't got out of loop
    ...
    while (cnt < 5) //is anybody graduated?
    {
        ...
        die_result = rolldie(turn);  //roll the dice
